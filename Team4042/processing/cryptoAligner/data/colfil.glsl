...
void main(void) {
  ...
  float h = step(abs(.67-hsv.x),.13);
  float s = step(.13,abs(hsv.y))*step(abs(hsv.y),1.);
  float v = step(.17,abs(hsv.z))*step(abs(hsv.z),.57);
  vec3 ocolor = vec3(mix(vec3(0.), vec3(0.,0.,1.), h*s*v));
