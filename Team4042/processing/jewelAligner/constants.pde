final float FUNCTION_A_A = 8001.3;
final float FUNCTION_A_B = 1.880;
final float FUNCTION_A_C = 301.5;
float[][] kernel = {{ -1, 0, 1},
                    { -2, 0, 2},
                    { -1, 0, 1}};
final float KERNEL_SCALER = 1;
final float TAPE_THRESHOLD = 2000;
final float FUNCTION_B_A = 1000.0;
final float FUNCTION_B_B = 1.7695;
final float FUNCTION_B_C = 1.1096;
final float FUNCTION_C_A = 1000.0;
final float FUNCTION_C_B = 1.7695;
final float FUNCTION_C_C = 1.1096;
final float FUNCTION_D_A = 1000.0;
final float FUNCTION_D_B = 1.7695;
final float FUNCTION_D_C = 1.1096;