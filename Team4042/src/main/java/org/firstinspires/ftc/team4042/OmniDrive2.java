...
public class OmniDrive2 extends Drive {
     ...
     */
    public OmniDrive2(HardwareMap hardwareMap, Telemetry tel) {
        //Initialize motors and gyro
        super(hardwareMap, tel);
        gyro = new RevGyro(hardwareMap, tel);
    }

    public OmniDrive2(HardwareMap hardwareMap, Telemetry tel, boolean verbose) {
    ...
    public void drive(boolean useEncoders, Gamepad gamepad1, Gamepad gamepad2, double speedFactor) {
        ...
        for (int n = 0; n <= 3; n++) {
            ...
            speedWheel[n] = xComp * Math.cos(Math.toRadians(m)) +
                    yComp * Math.sin(Math.toRadians(m)) + .7 * rot;
            m = (m + 90) % 360;
